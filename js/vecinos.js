...
if (screen.width <= 720) {
        window.location = "http://www.biodiversidad.gob.mx/ninos/movil/vecinos.html";
    }
  $("#maestro").find('a').attr('href', $("section:first").attr("data-maestros"));
  $('.arboles').on('click', function(){
    $('.mask, #modal').fadeIn();
    $('.imgArbol').css('background-image', 'url(' + $(this).attr('data-img') + ')');
    $('.descArbol').html($(this).attr('data-texto'));
    $('.descArbol i').css('color', $(this).parent().attr('data-color'));
  });
  resizeVecinos();
